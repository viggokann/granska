...
Scrutinizer::Scrutinizer(OutputMode mode) : ruleSet(NULL) {
    ...
    gramErrors = NULL;
    nGramErrors = 0;
    xAmbiguousNewWords = true; // f�r demon
    NewObj();
}
...
void Scrutinizer::PrintResult(std::ostream &out) {
    ...
    if (xPrintHTML) {
        out << "<H4>F�rklaringar till markeringarna</H4>"
            << "<LI>Misst�nkta omr�den i texten markeras med "
            << xRed << "r�tt" << xNoColor << "</LI>" << xEndl
            << "<LI>Ers�ttningsf�rslagen presenteras d�refter och �ndringar markeras med "
            << xGreen << "gr�nt" << xNoColor << "</LI>" << xEndl
            << "<LI>En kortfattad" << xBlue << " bl� " << xNoColor
            << "kommentar skrivs efter varje f�rslag</LI>" << xEndl
            << "<LI>En l�nk till utf�rligare information om feltypen"
            << " finns efter vissa kommentarer</LI>" << xEndl;
        if (!xSuggestionSameAsOriginalMeansFalseAlarm) 
            out << "<LI>Ett " << xRed << 'F' << xNoColor
                << " efter f�rslaget betyder att inga �ndringar gjorts, "
                << "vilket indikerar ett m�jligt falskt alarm</LI>" << xEndl;
        if (xAcceptNonImprovingCorrections)
            out << "<LI>Ett " << xRed << 'E' << xNoColor
                << " efter f�rslaget betyder att r�ttelsen kan ge ett annat fel</LI>" << xEndl;
        out << "<LI>Om en mening inneh�ller flera misst�nkta"
            << " felomr�den presenteras samma mening flera g�nger</LI>"
            << xEndl << xEndl
            << "Skicka g�rna synpukter till <A HREF=\"mailto:knutsson@nada.kth.se\">&lt;knutsson@nada.kth.se&gt;</A>"
            << "<HR WIDTH=\"100%\"></H1>";
    }
